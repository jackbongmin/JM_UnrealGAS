...
void UGA_Haste::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	if (!EffectClass)
	{
		EndAbility(Handle, ActorInfo, ActivationInfo, true, true);	// 이펙트 클래스가 없어서 실패처리
		return;
	}

	if (!CommitAbility(Handle, ActorInfo, ActivationInfo))
	{
		EndAbility(Handle, ActorInfo, ActivationInfo, true, true);	// 쿨타임이 안됐거나 코스트가 부족하다
		return;
	}
	...
	EndAbility(Handle, ActorInfo, ActivationInfo, true, false);	// 성공적으로 종료
