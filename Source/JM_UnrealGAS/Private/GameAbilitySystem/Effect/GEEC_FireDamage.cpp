...
struct FFireDamageStatics
{
	...
	FFireDamageStatics()
	{
		// UResourceAttributeSet의 Health 어트리뷰트를 타겟에서 캡처하도록 설정, 스냅샷은 사용하지 않음
		DEFINE_ATTRIBUTE_CAPTUREDEF(UResourceAttributeSet, Health, Target, false);

		// UStatusAttributeSet의 AttackPower 어트리뷰트를 소스에서 캡처하도록 설정, 공격 시점의 값을 가져오기(스냅샷 사용)
		DEFINE_ATTRIBUTE_CAPTUREDEF(UStatusAttributeSet, AttackPower, Source, true);
	}
};

// FFireDamageStatics을 싱글톤 인스턴스 반환하는 함수
...
void UGEEC_FireDamage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	if (TargetASC && SourceASC)
	{
		...
		//float Damage = Spec.GetSetByCallerMagnitude(Tag_EffectDamage, false, 1.0f);		// Tag_DataDamage에 설정된 값을 가져오기(못가져오면 1.0f)
		//if(Damage <= 0.0f)
		//{
		//	Damage = 1.0f;	// 혹시 음수면 최소값인 1로 설정
		//}

		if (SourceTags && SourceTags->HasTag(Tag_ElementFire)		// 공격자가 불속성이고
			&& TargetTags && TargetTags->HasTag(Tag_DebuffBurn))	// 피격자가 화상 디버프를 가지고 있으면
		{
			Damage *= 2.0f;	// 데미지 2배
		}

		// float Level = Spec.GetLevel();	// 이펙트의 레벨 가져오기
